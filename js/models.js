...
class StoryList {
  ...
  async addStory(user, newStory) {
    try {
      // Make a POST request to the API to add the new story
      const response = await axios({
        url: `${BASE_URL}/stories`,
        method: "POST",
        data: {
  ...
  async removeStory(user, storyId) {
    ...
    user.ownStories = user.ownStories.filter(id => id !== storyId);
  }
}
...
class User {
  ...
  function updateLocalStorageFavorites() {
    localStorage.setItem("favoriteStoryIds", JSON.stringify(currentUser.favorites));
  }
